...
ไฟล์สำคัญภายใน `bot/forex/core/bot`:
...
3. เปิด PowerShell ที่ `bot/forex/core` (จุดนี้เป็น import root ของต้นไม้ forex)
...
> **คำเตือน:** `bot/forex/main.bat` (ที่ root ของต้นไม้ ไม่ใช่ใน `core/`) เริ่ม **live trading ทันที**
> ส่วน `bot/forex/main.py` เป็นเมนูที่มีคำยืนยันก่อนส่งออเดอร์จริง

กด **Run Python File** ที่ `bot/forex/main.py` ใน VS Code ได้เลย หรือดับเบิลคลิก:
