...
`bot/trader.py` is where a plan becomes real orders and where every managed exit
...
from bot import guardrails, run, trader
from engine import market_hours, signals  # noqa: E402
from bot.broker import Broker, OrderRejected, SymbolSpec  # noqa: E402
from engine.instance_lock import LiveInstanceLock  # noqa: E402
...
from strategy.mt5_source import MT5Error  # noqa: E402
...
class WriteSpacingTests(unittest.TestCase):
    ...
    def test_a_second_write_waits_out_the_remaining_gap(self):
        ...
        slept = []
        with mock.patch("bot.broker.time.sleep", slept.append), \
                mock.patch("bot.broker.time.monotonic", return_value=100.5):
    ...
    def test_a_gap_already_served_does_not_sleep(self):
        ...
        broker._last_write = 100.0
        with mock.patch("bot.broker.time.sleep", slept.append), \
                mock.patch("bot.broker.time.monotonic", return_value=105.0):
            broker._pace()
        self.assertEqual(slept, [])
    ...
    def test_a_dry_run_never_pays_the_delay(self):
        ...
        broker._last_write = 100.0
        with mock.patch("bot.broker.time.sleep", slept.append), \
                mock.patch("bot.broker.time.monotonic", return_value=100.1):
            broker._pace()
        self.assertEqual(slept, [])
    ...
    def test_the_stamp_is_taken_when_the_request_leaves_not_when_it_returns(self):
        ...
        broker._spec = GOLD
        with mock.patch("bot.broker.time.monotonic", return_value=100.0):
