...
TREE_ROOT = TREE.parent  # bot/future/ -- one level too shallow to import core.* directly
...
class EntryPointTests(unittest.TestCase):
    ...
    def test_root_level_module_execution_explains_itself(self):
        # Regression: running the live entrypoint one level too shallow --
        # `python -m core.entrypoints.live` from bot/future/ instead of cd-ing
        # into bot/future/core/ first -- died with a bare ImportError from deep
        # inside the module, because each tree is its own import root and `bot`
        # bound to the wrong (nonexistent) package.
        result = self._run(["-m", "core.entrypoints.live", "--help"], TREE_ROOT)
        self.assertNotEqual(result.returncode, 0)
        combined = result.stdout + result.stderr
        self.assertIn("cd bot/future/core", combined)
    ...
    def test_future_entrypoint_from_root_explains_tree_boundary(self):
        result = self._run(
            ["-m", "core.entrypoints.main", "--status", "--offline"], TREE_ROOT)
        self.assertNotEqual(result.returncode, 0)
        combined = result.stdout + result.stderr
        self.assertIn("cd bot/future/core", combined)
        self.assertNotIn("ImportError", combined)
