...
คือ **XAUUSD M15 + M30, risk 0.40%/ไม้, capital-tier exit ที่ $30,000**
...
| `trader.py` | เปิดไม้ตาม exit mode, ดูแล pending, ปิดตาม timeout และลงผลเป็น R |
...
| Setup พร้อมกัน | 2 | หนึ่ง setup แบบ 3 TP นับเป็นหนึ่ง; ticket ที่ไม่รู้จักนับแยก |
...
| Initial balance < $30k | `fixed_tp3`: position เดียว ปิดเต็มจำนวนที่ TP3 (2R) |
| Initial balance ≥ $30k | 33/33/34 ที่ TP1/TP2/TP3 และเลื่อนส่วนที่เหลือไป BE หลัง TP1 |
...
SL/TP ถูกฝากไว้ที่โบรกเกอร์ ดังนั้นถ้าโปรแกรมหรืออินเทอร์เน็ตหยุด position ยังมีทางออก
ตามราคาที่ส่งไว้ แต่การเลื่อน BE เป็น client-side และตรวจทุก 180 วินาที จึงต้องเปิดโปรแกรมไว้
...
lot = risk cash ÷ (ระยะ SL × value/point) แล้วปัดตาม lot step ของ XAUUSD
ด้วยโหมด `nearest` โดยห้าม overshoot เกิน 15% จากเป้า ตัวอย่างสถานะบัญชี \$10,000
ล่าสุดคือ M15 0.02 lot ≈ 0.41% และ M30 0.01 lot ≈ 0.31%

ต่ำกว่า \$30,000 ใช้ position เดียวแบบ Fixed TP3 ตั้งแต่ \$30,000 ขึ้นไปต้องแบ่งได้ครบ
สาม position; หาก SL กว้างจนแบ่งไม่ได้ บอทปฏิเสธ setup แทนการเปลี่ยน exit เฉพาะไม้
