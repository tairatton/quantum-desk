...
ทุกอย่างที่ `bot.code.run` ทำได้ อยู่ในเมนูนี้ทั้งหมด ต่างกันแค่ตรงนี้ถามว่าจะทำอะไร
แทนที่จะต้องจำ flag
...
`bot.code.run` ยังใช้ได้เหมือนเดิมสำหรับคนที่ชอบ command line:

    python -m bot.code.run --status
    python -m bot.code.run --live
"""
...
def _journal() -> None:
    ...
    print("  outputs/backtests/technique_lab/XAUUSD/<TF>/report.json")
